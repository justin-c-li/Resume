...
double r=12;
double theta=45;
double lambda=-4; //recommended to go between -5 to 5 degrees for lambda and the lambda angle is restricted between -45 to 45 in main
double g=9.81;
double v=4500;
//##########################################//
...
void animate() {
	...
	state[6]+=rate[6]; //time needs to be first since it is used in calculation
	state[0]=rate[0]*state[6]+0.5*rate[3]*pow(state[6],2);
	state[1]=rate[1]*state[6]+0.5*rate[4]*pow(state[6],2);
	state[2]=rate[2]*state[6]+0.5*rate[5]*pow(state[6],2);
